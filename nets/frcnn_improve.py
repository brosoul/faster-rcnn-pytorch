...
from nets.classifier_improve import Resnet50RoIHead, VGG16RoIHead
...
class FasterRCNN(nn.Module):
    ...
    def forward(self, x, scale=1., mode="forward"):
        if mode == "forward":
            #     # 计算 map
            origin_imgs, rois, roi_indices = x

            #---------------------------------#
            #   计算输入图片的大小
            #---------------------------------#
            img_size        = origin_imgs.shape[2:]
            #---------------------------------#
            #   利用主干网络提取特征
            #---------------------------------#
            base_feature    = self.extractor.forward(origin_imgs)
