...
class FasterRCNNTrainer(nn.Module):
    ...
    def forward(self, imgs, bboxes, labels, anchors, scale):
        n           = imgs.shape[0]
        ...
        for anchor in anchors:
            tmp_rois.append(torch.from_numpy(anchor).float().type_as(imgs))
            
        # rois = torch.cat(tmp_rois, dim=0).type_as(imgs)
        rois = tmp_rois
        
        
        # rois      = torch.from_numpy(anchors).unsqueeze(0).float().to(imgs.device)
        # rpn_locs, rpn_scores, rois, roi_indices, anchor = self.model_train(x = [base_feature, img_size], scale = scale, mode = 'rpn')
        roi_indices = list()
        for i in range(n):
            batch_index = i * torch.ones((rois[i].shape[0],))
            roi_indices.append(batch_index.unsqueeze(0).type_as(imgs))
            
        # roi_indices = torch.cat(roi_indices, dim=0).type_as(imgs)
        ...
        for i in range(n):
            ...
            # sample_rois.append(torch.Tensor(sample_roi).float())
            # sample_indexes.append(torch.ones(len(sample_roi)).float() * roi_indices[i][0])
            # gt_roi_locs.append(torch.Tensor(gt_roi_loc).float())
            # gt_roi_labels.append(torch.Tensor(gt_roi_label).long())
            
            sample_rois.append(torch.Tensor(sample_roi).type_as(imgs))
            sample_indexes.append(torch.ones(len(sample_roi)).type_as(imgs) * roi_indices[i][0])
            gt_roi_locs.append(torch.Tensor(gt_roi_loc).type_as(imgs))
            gt_roi_labels.append(torch.Tensor(gt_roi_label).type_as(imgs).long())
        ...
        for i in range(n):
            ...
            n_sample = roi_cls_locs[i].shape[0]
            if n_sample == 0:
                continue
        ...
        losses = [roi_loc_loss_all/n, roi_cls_loss_all/n]
        losses = losses + [sum(losses)]
        return losses

    def train_step(self, imgs, bboxes, labels, anchors, scale, fp16=False, scaler=None):
        self.optimizer.zero_grad()
        if not fp16:
            losses = self.forward(imgs, bboxes, labels, anchors, scale)
            losses[-1].backward()
            self.optimizer.step()
        else:
            raise("fp16 not imple")
